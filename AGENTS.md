...
- `yarn build:website-resume [--variant senior-devops-engineer-en]` – executes `resume/build-website-resume.ts`, producing `resume/artifacts/resume.html` and copying it into `frontend/resume.html`.
...
TypeScript and JavaScript use 2-space indentation, single quotes, and trailing commas; run `npx prettier --write` (auto-triggered via `lint-staged` on `*.ts`). Filenames stay kebab-case (`automation-platform-engineer-en.ts`). CSS utility rules belong in `frontend/additional.css`, while `styles.css` holds base typography/layout. Resume variant IDs follow `<role>-<language>` and should match filenames in `resume/src/data/common/`.
